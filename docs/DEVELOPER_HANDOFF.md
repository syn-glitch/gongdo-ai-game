<!--
 ...
 @agent       📝 꼼꼼이 (Docs Team Lead) · 송PO 직접 patch (BUNKER-2026-04-28-002 사이클 동기)
 @ordered-by  용남 대표
 @description 외부 개발자 핸드오프 문서 — 이슈 발생 시 사용자(바이브코더) 가 자체 해결 못 하면 개발자가 본 문서만 보고 진단·해결하기 위한 단일 진실 원천.
 @audience    문유나 / 김민석 (외부 개발자)
 @scope       gongdo-ai-game (https://gongdo-ai-game.vercel.app/) v0.4.2-lesson2-baseline 기준

 ── 변경 이력 ──────────────────────────
 v1.1.1 | 2026-04-28 | 꼼꼼이 (송PO 직접 patch) | BUNKER-2026-04-28-002 사이클 통합 — 안정 tag v0.4.2-lesson2-baseline, lesson2.md 1차시 baseline 정렬 (첫 진입 본문 "파란 우주선" 으로 통일). §4.2 hero line 예시 갱신 + §8.4 매칭표 추가
...
>
> **현재 안정 버전**: `v0.4.2-lesson2-baseline` (2026-04-28 commit `07f32df`) — 직전 안정: `v0.4.1-feature-gate` (`61cac7c`) → `v0.4.0-pilot` (`ad24a31`)
...
| **안정 tag** | [`v0.4.2-lesson2-baseline`](https://github.com/syn-glitch/gongdo-ai-game/releases/tag/v0.4.2-lesson2-baseline) (직전: `v0.4.1-feature-gate` → `v0.4.0-pilot`) |
...
- lesson2.md: `- 주인공: 파란 우주선` ✅ (v3.1.0 부터 1차시 baseline 정렬, BUNKER-2026-04-28-002 — 첫 진입 시 IP 캐릭터 미배치, 학생이 캐릭터 활성화 + IP 클릭으로 hero line patch 학습 흐름)
...
|---|---|---|---|
| `v0.4.2-lesson2-baseline` | `07f32df` | `dpl_CrHtngpPzi2yFUXX5sxs9yrSFjZd` | 2026-04-28 (lesson2.md 첫 진입 본문 1차시 baseline 정렬 — "파란 우주선" 시작) |
...
본 문서 v1.1.1 기준 (현재 안정 `v0.4.2-lesson2-baseline` = `07f32df`, 직전 안정 `v0.4.1-feature-gate` = `61cac7c` → `v0.4.0-pilot` = `ad24a31`).

### BUNKER-2026-04-28-002 사이클 (현재 안정)

| commit | 변경 |
|---|---|
| `07f32df` | docs(lesson2): v3.0.0 → v3.1.0 — lesson2.md 첫 진입 본문을 1차시 baseline 과 정렬. 게임 명세 영역 `### 주인공 / - 주인공: 파란 우주선` 으로 시작(이전: 토리), 배경도 1차시(검은 우주)로 교체. 적은 lesson2 의 "주사위 모양 장애물" 그대로 유지(다양성). 페다고지 영역 일부 정리(친구들 표 삭제, 예시 IP 단어 → "주인공" 통일, "'펑'→'띠링'" 1줄 삭제, 조작·규칙 추가). 학생 학습 흐름 정상화: 파란 우주선 시작 → 자유 텍스트 시도 → 캐릭터 활성화 → IP 클릭으로 hero line patch ← **현재 안정 (v0.4.2-lesson2-baseline)** |

### BUNKER-2026-04-28-001 사이클

| commit | 변경 |
|---|---|
| `61cac7c` | feat(gate): S17 파일럿 후순위 4건 비활성화 — 사이드바 3·4차시 + 도구바 음악·예시. manifest.json `lessons[].enabled` + `featureFlags{music,example}.enabled` 단일 토글 진실. 캐릭터 게이트 코드 비건드림(회귀 위험 0). 키워드 미정 fallback (모달 [확인] 시 토스트 "3,4교시에 만나요" + 닫힘) (이전 안정 `v0.4.1-feature-gate`) |
...
**문서 끝.**
**버전**: v1.1.1 (2026-04-28, 꼼꼼이 / 송PO 직접 patch)
**다음 갱신 권장**: lesson1·3·4 본문 정리 사이클 진행 시 §4.5 lesson 자료 표준 / §11 변경 이력 동기. 음악·예시 활성화 키워드 작명 시 v1.2.0. 새 안정 tag(`v0.5.0+`) 출시 시.
