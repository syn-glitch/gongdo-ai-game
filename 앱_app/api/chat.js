 ...
 * @agent       👧 클로이 FE (자비스 개발팀) · 지시: 자비스 PO (넷마블 IP 제거 작업)
 * @ordered-by  용남 대표
 * @description /api/chat — mode="generator" HTML 게임 생성 · mode="tutor" 학생 질문 응답.
 *              모델: claude-haiku-4-5-20251001 · Prompt Caching 적용.
 *
 * @change-summary
 *   AS-IS: v1.6 — 넷마블 IP 캐릭터(ㅋㅋ/토리/밥/레옹) PNG URL + 이모지(🦸/👒/🐰/🦁) 매핑
 *   TO-BE: v1.7 — 공도 자체 마스코트(별이/구름이/물방울/새싹이) SVG URL + 이모지(⭐/☁️/💧/🌱) 매핑
 *
 * @features
 *   - [수정] SYSTEM_GENERATOR 캐릭터 매핑표 4종 — 신규 마스코트로 전환
 *   - [수정] CHAR_TO_EMOJI 후처리 매핑 — 별이/구름이/물방울/새싹이
 *   - [수정] hasCharacterUrl 정규식 .png → .svg
 *   - [유지] HTML 추출 안전 fallback / forceEmojiCharacters / injectBgmIntoGame 등 모든 인프라
 *
 * ── 변경 이력 ──────────────────────────
 * v1.7.0 | 2026-05-05 | 클로이 | 넷마블 IP 캐릭터 전면 제거 + 자체 마스코트 4종 교체
...
- 별이 → ⭐ / 구름이 → ☁️ / 물방울 → 💧 / 새싹이 → 🌱
- 일반 적 → 👾 👽 / 보스 → 👹 / 아이템 → 🌟 💎 🍎 / 폭발 → 💥
- 캐릭터 이름(별이·구름이 등)을 fillText 인자로 그리면 안 됨 (이모지로 치환됨)

❌ 금지: ctx.fillText('별이', x, y)   ← 캐릭터 이름 글자 그리기 금지
...
function drawPlayer(ctx, x, y) {
  ...
  ctx.fillText('⭐', x, y);  // 문서에 "주인공: 별이" 이면 ⭐
}
...
【공도 마스코트 이미지 참조 — 매우 중요】
문서에 캐릭터가 언급되면 반드시 **학생 문서에 적힌 절대 URL** 을 그대로 사용합니다. 상대 경로(/에셋_assets/...) 를 임의로 만들어 쓰지 마세요 — iframe srcdoc 에서 작동하지 않습니다.

예시: 문서에 "- 주인공: 별이 (이미지: https://gongdo-ai-game.vercel.app/에셋_assets/캐릭터_characters/star_idle.svg)" 이라고 적혀있으면, 해당 URL 을 그대로 img 태그의 src 속성에 사용합니다 (예: img src="https://gongdo-ai-game.vercel.app/에셋_assets/캐릭터_characters/star_idle.svg").

만약 문서에 이미지 URL 이 명시되지 않았다면 이모지 폴백만 사용:
- 별이 → ⭐ / 구름이 → ☁️ / 물방울 → 💧 / 새싹이 → 🌱

【이모지 폴백 매핑 — onerror·이미지 URL 미명시 시에만】
SVG 가 로드 실패하거나 학생 문서에 이미지 URL 이 아예 없을 때만 사용:
- 별이   → ⭐
- 구름이 → ☁️
- 물방울 → 💧
- 새싹이 → 🌱
...
★ 위 \`<STUDENT_DOCUMENT_IMAGE_URL>\` 과 \`<EMOJI_FALLBACK>\` 은 **placeholder** 입니다.
**절대 \`star_idle.svg\` / \`⭐\` 를 그대로 복사하지 마세요.** 학생이 어떤 캐릭터를 골랐는지 문서에서 읽고:

| 학생 문서의 캐릭터 | playerImg.src (URL 그대로 복사) | 이모지 폴백 |
|------------------|--------------------------------|-------------|
| 별이   | https://gongdo-ai-game.vercel.app/에셋_assets/캐릭터_characters/star_idle.svg   | ⭐ |
| 구름이 | https://gongdo-ai-game.vercel.app/에셋_assets/캐릭터_characters/cloud_idle.svg  | ☁️ |
| 물방울 | https://gongdo-ai-game.vercel.app/에셋_assets/캐릭터_characters/drop_idle.svg   | 💧 |
| 새싹이 | https://gongdo-ai-game.vercel.app/에셋_assets/캐릭터_characters/sprout_idle.svg | 🌱 |

❌ 위반 사례 (절대 금지):
- 학생이 "주인공: 물방울" 이라 적었는데 코드에 \`star_idle.svg\` 가 들어가는 것 ← 가장 흔한 실수
- 4명 모두 같은 SVG 사용
...
\`\`\`html
<img src="https://gongdo-ai-game.vercel.app/에셋_assets/캐릭터_characters/star_idle.svg"
...
- 학생 문서의 정확한 표현을 그대로 인용 (예: "주인공: 별이" → "주인공: 별이")

■ 좋은 예시
\`\`\`js
// 📝 "주인공: 별이" → 너의 주인공 별이를 화면에 그리는 묶음이야!
function drawPlayer(ctx, x, y) {
  ctx.fillText('⭐', x, y);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (mode === 'generator' && html) {
      ...
      const hasCharacterUrl = /에셋_assets\/캐릭터_characters\/[a-z_]+\.svg/i.test(html);
